...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // ✅ EFFECT PRINCIPAL: Inicializar autenticação
  useEffect(() => {
    let mounted = true;
    ...
    // ✅ LISTENER: Observar mudanças de autenticação
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (event === "SIGNED_IN" && session?.user) {
          ...
          // ✅ NOVO: Capturar a sessão inicial após redirecionamento
          console.log("🆕 INITIAL_SESSION detectada");
          if (session?.user) {
            console.log("✅ INITIAL_SESSION com usuário, buscando perfil");
            await fetchUserProfile(session.user);
          } else {
            console.log("❌ INITIAL_SESSION sem usuário");
            setUser(null);
            setIsLoading(false);
          }
        } else if (event === "TOKEN_REFRESHED") {
          // ✅ NOVO: Quando token é renovado
          console.log("🔄 TOKEN_REFRESHED");
          if (session?.user && !user) {
            console.log("✅ TOKEN_REFRESHED com usuário novo");
            await fetchUserProfile(session.user);
          }
        }
      }
