...
const classesByDay = daysOfWeek.reduce((acc, day) => {
  acc[day] = classes?.filter((c: any) =>
    c.day_of_week?.toLowerCase().includes(day.toLowerCase()) // ✅ Corrigido
  ) || []
  return acc
                  ...
                  {dayClasses.map((classData: any) => (
                    <Card key={classData.id} className="p-6">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="space-y-1 text-sm text-muted-foreground">
                            <div className="flex items-center gap-2">
                              ...
                              <span>Instrutor: {classData.instructor_name}</span>
                            </div>
                            
                            <div className="flex items-center gap-2">
                              <Clock className="h-4 w-4" />
                              <span>
                                {classData.schedule_time} • {classData.duration_minutes} min
                              </span>
                            </div>
                        ...
                        <Button
                          size="sm"
                          onClick={() => handleCheckin(classData.id, classData.name)}
                        >
